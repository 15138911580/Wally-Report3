	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(method.equals("reg2")){//会员注册 reg2.jsp
			...
			if(username.trim().equals("admin")){
				request.setAttribute("username", username);
				request.setAttribute("message", "Illegal username, please re-select！");
				...
				if(flag == Constant.SUCCESS){
					...
					request.setAttribute("username", username);
					request.setAttribute("message", "Sorry, the username already exists, please re-select！");
					request.getRequestDispatcher("reg2.jsp").forward(request, response);
				}
				else{
			...
			if(username.trim().equals("admin")){
				...
				if(flag == Constant.SUCCESS){
					request.setAttribute("username", username);
					request.setAttribute("message", "Congratulations, this name can be used！");
					request.getRequestDispatcher("reg2.jsp").forward(request, response);
				}
				else if(flag == Constant.SAME_NAME){
					request.setAttribute("username", username);
			...
			if(member==null||type==null){
				...
				switch (flag){
					case Constant.SUCCESS:
						request.setAttribute("message", "Password reset complete！");
						request.getRequestDispatcher("member/info/editpwd.jsp").forward(request, response);
						break;
					case Constant.PASSWORD_ERROR:
						request.setAttribute("message", "The original password is incorrect, please confirm the permission！");
						request.getRequestDispatcher("member/info/editpwd.jsp").forward(request, response);
						break;
		 ...
		 *********************************************/
			...
			if(flag == Constant.SUCCESS){
					...
					request.setAttribute("message", "registration success! Congratulations on becoming"+sitename+"Registered member！");
			...
			if(info.trim().equals("error")){
				...
				request.setAttribute("message", "Your new password is"+info+"，Please login to modify！");
				request.getRequestDispatcher("login.jsp").forward(request, response);
			}
		}
		else{
